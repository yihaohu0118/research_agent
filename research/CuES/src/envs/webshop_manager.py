...
class WebshopEnvironmentManager:
    """Webshop environment manager wrapping EnvClient"""
    
    def __init__(self, server_url="http://localhost:8080", env_type="webshop", client=None):
    ...
    def reset(self, task_id=None, env_id=None, stage="stage1") -> Tuple[str, Dict[str, Any]]:
        ...
        try:
            ...
            # Webshop specific initialization
            if stage == "stage1":
                current_observation = [
                ""
                ]
            ...
            logger.info(f"Created Webshop instance: {self.instance_id}, task: {task_id}")
            ...
            logger.error(f"Reset Webshop environment failed: {e}")
            raise
    ...
    def step(self, action: str, task_description: str = None, query: str = None) -> Tuple[str, float, bool, Dict[str, Any]]:
        ...
        try:
            ...
            observation = "Please continue exploring based on the existing valid actions. \n"
    ...
    def close(self):
        ...
        if self.instance_id:
            try:
                ...
                logger.info(f"Released Webshop instance: {self.instance_id}")
            except Exception as e:
...
# Usage example
def main():
    """Webshop Environment Manager usage example"""
    manager = WebshopEnvironmentManager()

    try:
        # Get available tasks
        tasks = manager.get_available_tasks()
        print(f"Available tasks: {tasks[:3]}...")  # Display first 3 tasks
